...
# specify paths in 
path_output_all_or_none <- '../../output/analysis/vector_control/output_files/all_or_none/prop_recurr/'
...
eir_equil <- sort(unique(output_all_or_none$EIR_equil))           
...
offset <- seq(from = -0.225, to = 0.225, length.out = nrow(scenarios_vector_control) * 1)

jpeg(filename = '../../output/figs/fig_S5.jpg', width = 8, height = 11, units = 'in', res = 500)
...
for(ee in 1:length(eir_equil))
{
  ...
  {
    rect(xleft = ee + offset[1 + 1 * (vv-1)] - (rectwidth/2), xright = ee + offset[1 + 1 * (vv-1)] + (rectwidth/2),
         ybottom = 0, ytop = prop_all_default[[vv]][2,ee], col = palette[vv])
    segments(x0 = ee + offset[1 + 1 * (vv-1)], y0 = prop_all_default[[vv]][1,ee], y1 = prop_all_default[[vv]][3,ee],
             col = '#222222', lwd = 1.5)
  }
}
box()
...
mtext(side = 2, line = 2.3, 'Relapses (%)')
...
legend('topright', pch = c(rep(16, 4)), col = c(palette),
       legend = c('No Vector Control', 'LLINs only', 'IRS only', 'LLINs and IRS'),
       bty = 'n', pt.cex = 2)
...
for(ee in 1:length(eir_equil))
{
  for(vv in 1:nrow(scenarios_vector_control))
  {
    ...
    segments(x0 = ee + offset[1 + 1 * (vv-1)], y0 = prop_all_endophagic_bed[[vv]][1,ee], y1 = prop_all_endophagic_bed[[vv]][3,ee],
             col = '#222222', lwd = 1.5)
  }
}
box()
...
for(ee in 1:length(eir_equil))
{
  for(vv in 1:nrow(scenarios_vector_control))
  {
    ...
    segments(x0 = ee + offset[1 + 1 * (vv-1)], y0 = prop_all_endophagic_indoors[[vv]][1,ee], y1 = prop_all_endophagic_indoors[[vv]][3,ee],
             col = '#222222', lwd = 1.5)
  }
}
box()
...
for(ee in 1:length(eir_equil))
{
  for(vv in 1:nrow(scenarios_vector_control))
  {
    ...
    segments(x0 = ee + offset[1 + 1 * (vv-1)], y0 = prop_all_exophagic[[vv]][1,ee], y1 = prop_all_exophagic[[vv]][3,ee],
             col = '#222222', lwd = 1.5)
  }
}
box()
