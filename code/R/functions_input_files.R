...
# function to generate trial input file
genTrialFile = function(output_file_path, # path to which the file should be generated 
                        treatment_arm_sample_size, # sample size of the treatment arm
                        placebo_arm_sample_size, # sample size of the placebo arm 
                        recruitment_start_date, # date on which recruitment to the trial should begin
                        trial_duration, # number of days for each the trial will last for each person 
                        dropout_rate, # rate of dropout of participants 
                        trial_PQ_eff_stratum_1, # efficacy of PQ in the clinical trial for stratum 1
                        trial_PQ_eff_stratum_2, # efficacy of PQ in the clinical trial for stratum 2
                        trial_PQ_prop_stratum_1, # proportion of individuals belonging to stratum 1 
                        trial_PQ_prop_stratum_2, # proportion of individuals belonging to stratum 2 
                        trial_PQ_lowage, # minimum age for enrollment in the clinical trial
                        trial_PQ_proph, # duration (in days) of prophylaxis provided with treatment
                        trial_observation_period, # duration (in days) of observation period for treatment 
                        is_LLIN_distributed, # boolean to specify whether or not LLINs are distributed during enrollment
                        is_IRS_administered, # boolean to specify whether or not IRS is administered at enrollment
                        G6PD_activity_threshold, # threshold G6PD activity level for enrollment 
                        followup_dates) # dates of followup during trial 
{
  # get parameter names and files 
  parameter_names <- c('treatment_arm_sample_size',
                       'placebo_arm_sample_size',
                       'recruitment_start_date',
                       'trial_duration',
                       'dropout_rate',
                       'trial_PQ_eff_stratum_1',
                       'trial_PQ_eff_stratum_2',
                       'trial_PQ_prop_stratum_1',
                       'trial_PQ_prop_stratum_2',
                       'trial_PQ_lowage',
                       'trial_PQ_proph',
                       'is_LLIN_distributed',
                       'is_IRS_administered',
                       'G6PD_activity_threshold',
                       paste('followup_date_', 1:length(followup_dates), sep = ''))
  
  parameter_values <- c(treatment_arm_sample_size,
                        placebo_arm_sample_size,
                        recruitment_start_date,
                        trial_duration,
                        dropout_rate,
                        ...
                        trial_PQ_lowage,
                        trial_PQ_proph,
                        as.integer(is_LLIN_distributed),
                        as.integer(is_IRS_administered),
                        G6PD_activity_threshold,
                        followup_dates)
  
  # convert to df and write out to file 
  df <- data.frame(parameter_names, parameter_values)
  write.table(x = df, file = output_file_path, sep = ',', row.names = F, col.names = F, quote = F)
}


# function to generate model parameter input file 
genModelParamsFile <- function(template_file_path = '../../data/templates/model_parameters.txt',
                               output_file_path, # path to which file should be generated
                               N_part = 50000, # number of participants in simulation
                               EIR_equil, # EIR equilibrium
                               sig_het = 1.292285, # degree of heterogeneity in exposure 
                               ff = 0.01538462, # relapse rate (1/day)
                               gamma_L = 0.002610966, # rate of clearance from the liver (1/day)
                               P_occup = 0.37, # proportion of men engaging in occupational hazard
                               risk_occup = 0, # occupational risk hazard 
                               rho_age = 0.85, # age-dependent biting 
                               start_time = 2019, # year for simulation to start
                               end_time = 2030, # year for simulation to end
                               burnin_time = 0, # number of burnin years
                               CM_PQ_proph = 8, # number of days of prophylaxis from PQ
                               CM_CQ_proph = 28, # number of days of prophylaxis from CQ
                               CM_regimen = 0, # Case management regimen (CQ only)
                               CM_PQ_eff = 1, # PQ efficacy
                               CM_PQ_adhere = 1, # PQ adherence
                               CM_PQ_prop_stratum_1 = 0, # proportion of individuals in PQ stratum 1 
                               CM_PQ_eff_stratum_1 = 0, # efficacy of PQ in stratum 1 
                               CM_PQ_eff_stratum_2 = 0, # efficacy of PQ in stratum 2 
                               G6PD_prev = 0, # G6PD prevalence 
                               CYP2D6_prev = 0) # CYP2D6 prevalence
{
  # load data template 
  template <- read.table(template_file_path)
  
  # update parameters 
  template[which(template[,1] == 'N_part'),2] <- N_part
  template[which(template[,1] == 'EIR_equil'),2] <- EIR_equil
  template[which(template[,1] == 'sig_het'), 2] <- sig_het
  template[which(template[,1] == 'ff'), 2] <- ff
  template[which(template[,1] == 'gamma_L'),2] <- gamma_L
  template[which(template[,1] == 'P_occup'),2] <- P_occup
  template[which(template[,1] == 'risk_occup'),2] <- risk_occup
  template[which(template[,1] == 'rho_age'),2] <- rho_age
  template[which(template[,1] == 'start_time'),2] <- start_time
  template[which(template[,1] == 'end_time'),2] <- end_time
  template[which(template[,1] == 'burnin_time'),2] <- burnin_time
  template[which(template[,1] == 'CM_PQ_proph'),2] <- CM_PQ_proph
  template[which(template[,1] == 'CM_CQ_proph'),2] <- CM_CQ_proph
  template[which(template[,1] == 'CM_regimen'),2] <- CM_regimen
  template[which(template[,1] == 'CM_PQ_eff'),2] <- CM_PQ_eff
  template[which(template[,1] == 'CM_PQ_adhere'),2] <- CM_PQ_adhere
  template[which(template[,1] == 'CM_PQ_prop_stratum_1'),2] <- CM_PQ_prop_stratum_1
  template[which(template[,1] == 'CM_PQ_eff_stratum_1'),2] <- CM_PQ_eff_stratum_1
  template[which(template[,1] == 'CM_PQ_eff_stratum_2'),2] <- CM_PQ_eff_stratum_2
  template[which(template[,1] == 'G6PD_prev'),2] <- G6PD_prev
  template[which(template[,1] == 'CYP2D6_prev'),2] <- CYP2D6_prev
  
  # convert participants element to integer to avoid writing out in scientific notation 
  template[which(template[,1] == 'N_part'),2] <- as.integer(template[which(template[,1] == 'N_part'),2])
  
  # write to file 
  write.table(x = template, 
...
# function to generate mosquito input file 
genMosqFile = function(template_file_path = '../../data/templates/domestic_mosquitoes.txt',
                       output_file_path, # path to which the file should be generated
                       kappa_seas = 0, # parameter governing the degree of seasonality 
                       PSI_indoors, # proportion of bites occurring indoors
                       PSI_bed) # proportion of bites occurring in bed
{
  # check that the proportion of bites in occurring in bed is not greater than the proportion of bites occurring indoors
  if(PSI_bed > PSI_indoors)
  {
    stop('Proportion of bites in bed cannot be greater than indoors')
  }
  ...
  # update parameters 
  template[which(template[,1] == 'kappa_seas'),2] <- kappa_seas
  template[which(template[,1] == 'PSI_indoors'),2] <- PSI_indoors
  template[which(template[,1] == 'PSI_bed'),2] <- PSI_bed
...
# function to generate intervention coverage input file 
genInterventionFile = function(
  output_file_path, # path to which the file should be generated
  ...
  ##     ##  Includes information for coverage, efficacy and
  #########  duration of prophylaxis.
  CM1_years           = c( ),     ## Start of case management regimen
  CM1_cover           = c( ),     ## Coverage of chloroquine (accounts for proportion seeking care)
  CM1_CQ_eff          = c( ),    ## chloroquine drug efficacy (on its own)
  CM1_CQ_eff_wPQ      = c( ),    ## chloroquine drug efficacy (co-administered with PQ)
  CM1_CQ_proph        = c( ),       ## duration of prophylaxis of chloroquine 
  CM1_PQ_eff          = c( ),    ## primaquine efficacy
  CM1_PQ_proph        = c( ),        ## duration of primaquine prophylaxis (number of days + 1)
  CM1_PQ_adhere       = c( ),   ## adherence to full primaquine regimen
  CM1_PQ_lowage       = c( ),      ## youngest age for primaquine treatment regimen
  CM1_PQ_G6PD_risk    = c( ),        ## Risk in G6PD-deficient individuals (0 = no risk; 1 = risk)
  CM1_PQ_CYP2D6_risk  = c( ),        ## Risk of not working in low CYP2D6 metabolizers  (0 = no risk; 1 = risk, i.e. doesn't work)
  CM1_PQ_preg_risk    = c( ),        ## Risk in pregnant women  (0 = no risk; 1 = risk)
  CM1_G6PD_test       = c( ),	       ## Is G6PD testing implemented in case management (0 = no; 1 = yes)
  ...
  STAT1_years           = c( ),       ## Time of MDA programme
  STAT1_cover           = c( ),        ## Coverage
  STAT1_sens            = c( ),       ## Sensitivity of diagnostic tool
  STAT1_spec            = c( ),        ## Specificity of diagnostic tool
  STAT1_RDT_PCR         = c( ),          ## What diagnostic tool (extra parasitological testing)): 0 = nothing; 1 for RDT(= LM); 2 for PCR
  STAT1_CQ_eff          = c( ),      ## Efficacy of chloroquine (on its own)
  STAT1_CQ_eff_wPQ      = c( ),      ## Efficacy of chloroquine (co-administered with PQ)
  STAT1_CQ_proph        = c( ),         ## Duration of chloroquine prophylaxis
  STAT1_PQ_eff          = c( ),      ## Efficacy of primaquine
  STAT1_PQ_proph        = c( ),          ## Duration of primaquine prophylaxis
  STAT1_PQ_adhere       = c( ),     ## Primaquine adherence
  STAT1_PQ_lowage       = c( ),       ## Lower age limit for primaquine treatment (in days)
  STAT1_PQ_G6PD_risk    = c( ),          ## Risk in G6PD - deficient individuals
  STAT1_PQ_CYP2D6_risk  = c( ),          ## Risk of not working in low CYP2D6 metabolizers
  STAT1_PQ_preg_risk    = c( ),          ## Risk in pregnant women
  STAT1_G6PD_test       = c( ),          ## G6PD testing for PQ eligible cases  (0 = no test; 1 = test)
  ...
  INT_cov <- matrix(-1, nrow=max(length(years),1), ncol=151)
  ...
  {
    if( length(CM0_years) > 0 & INT_cov[i,1] %in% CM0_years )
    {
      if(length(CM0_cover) > 0)
        INT_cov[i,2] = CM0_cover[which(CM0_years==INT_cov[i,1])]
      if(length(CM0_CQ_eff) > 0)
        INT_cov[i,3] = CM0_CQ_eff[which(CM0_years==INT_cov[i,1])]
      if(length(CM0_CQ_proph) > 0)
        INT_cov[i,4] = CM0_CQ_proph[which(CM0_years==INT_cov[i,1])]
    }
    
    if( length(CM1_years) > 0 & INT_cov[i,1] %in% CM1_years )
    {
    ...
    if( length(CM2_years) > 0 & INT_cov[i,1] %in% CM2_years )
    {
    ...
    if( length(LLIN_years) > 0 & INT_cov[i,1] %in% LLIN_years )
    {
      INT_cov[i,36] = LLIN_cover[which(LLIN_years==INT_cov[i,1])]
    }
    
    if( length(IRS_years) > 0 & INT_cov[i,1] %in% IRS_years )
    {
      INT_cov[i,37] = IRS_cover[which(IRS_years==INT_cov[i,1])]
    }
    
    if( length(MDA0_years) > 0 & INT_cov[i,1] %in% MDA0_years )
    {
    ...
    if( length(MDA1_years) > 0 & INT_cov[i,1] %in% MDA1_years )
    {
    ...
    if( length(MDA2_years) > 0 & INT_cov[i,1] %in% MDA2_years )
    {
    ...
    if( length(MSAT0_years) > 0 & INT_cov[i,1] %in% MSAT0_years )
    {
    ...
    if( length(MSAT1_years) > 0 & INT_cov[i,1] %in% MSAT1_years )
    {
    ...
    if( length(MSAT2_years) > 0 & INT_cov[i,1] %in% MSAT2_years )
    {
    ...
    if( length(STAT1_years) > 0 & INT_cov[i,1] %in% STAT1_years )
    {
    ...
    if( length(STAT2_years) > 0 & INT_cov[i,1] %in% STAT2_years )
    {
    ...
    if( length(IVM_years) > 0 & INT_cov[i,1] %in% IVM_years )
    {
  ...
  write.table(INT_cov, file=output_file_path, col.names=FALSE)
}
